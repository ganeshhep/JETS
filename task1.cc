...
int main()
{
  ...
  TFile *output = new TFile("task1.root", "recreate");
  TTree *tree = new TTree("tree", "tree"); 
  
  int idAbs, event, size, event_num;
  ...
  for (int i = 0; i < nevents; i++)
  {
    ...
    for (int j = 0; j < entries; j++)
    {
      ...
      for (int k = 0; k < 28; k++){
	    ...
	    int daughter_idAbs = pythia.event[vec_daughter_list.at(l)].idAbs();
            for (int m = 0; m < 28; m++){
	      if (daughter_idAbs == bCodes[m]){
		isbHadron = 0;
	      }
	    }
	  }
